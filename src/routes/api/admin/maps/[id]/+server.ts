...
export const PATCH: RequestHandler = async ({ locals, params, request }) => {
  ...
  if (body.physical_description !== undefined)
    updateData.physical_description = body.physical_description || null;
  ...
  if (body.holding_institution !== undefined)
    updateData.holding_institution = body.holding_institution || null;
  ...
  if (updateData.iiif_image && body.allmaps_id === undefined) {
    ...
    if (!existing?.allmaps_id) {
      try {
        updateData.allmaps_id = await deriveAllmapsId(updateData.iiif_image);
      } catch (e) {
...
export const DELETE: RequestHandler = async ({ locals, params }) => {
  ...
  const { error: dbError } = await adminSupabase.from('maps').delete().eq('id', mapId);
