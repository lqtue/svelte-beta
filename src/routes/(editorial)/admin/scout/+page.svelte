      ...
      <section class="grid">
        {#each rows as c (c.id)}
          <article class="card" class:selected={selected.has(c.id)}>
            ...
            <a
              href={c.source_url || c.manifest_url || '#'}
              target="_blank"
              rel="noopener"
              class="thumb-link"
            >
              <img
                ...
                loading="lazy"
                on:error={(e) => {
                  (e.target as HTMLImageElement).src = placeholderThumb(c);
                }}
              />
            </a>
            <div class="card-body">
              ...
              <div class="actions">
                {#if c.status === 'pending'}
                  <button class="btn-good" on:click={() => setStatus(c.id, 'approved')}
                    >Approve</button
                  >
                  <button class="btn-bad" on:click={() => setStatus(c.id, 'rejected')}
                    >Reject</button
                  >
                {:else}
                  <span class="status-badge {c.status}">{c.status}</span>
                  <button class="btn-link" on:click={() => setStatus(c.id, 'pending')}
                    >↺ Revert</button
                  >
                {/if}
              </div>
...
<style>
  ...
  .chip.score[data-score='high'] {
    background: #2e7d32;
    color: #fff;
  }
  .chip.score[data-score='mid'] {
    background: #ed8c00;
    color: #fff;
  }
  .chip.score[data-score='low'] {
    background: #999;
    color: #fff;
  }
  .chip.score[data-score='neg'] {
    background: #b71c1c;
    color: #fff;
  }
  .holder {
