<script lang="ts">
  ...
  let extraPairs: { key: string; value: string }[] = Object.entries(extra_metadata).map(
  ...
  $: {
    ...
    if (legend.length > 0 && typeof legend[0] === 'object') {
      ...
      labelLegendText = legend
        .map((l: any) => (typeof l === 'string' ? l : `${l.val} | ${l.label}`))
        .join('\n');
    } else {
  ...
  async function handleMirrorToR2() {
    ...
    try {
      ...
      map.thumbnail =
        (mirrorResult as any).thumbnail || `${mirrorResult.iiif_image}/full/256,/0/default.jpg`;
  ...
  async function handleSave() {
    ...
    try {
      ...
      const parsedCategories = labelCategories
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    <div class="modal-body">
      ...
      {#if activeTab === 'about'}
        ...
        <div class="extra-meta-section">
          ...
          <button
            type="button"
            class="btn-add-pair"
            on:click={() => (extraPairs = [...extraPairs, { key: '', value: '' }])}
            >+ Add field</button
          >
        </div>
      {:else if activeTab === 'source'}
        ...
        <div class="form-grid">
          ...
          <label class="form-label">
            ...
            <input
              type="text"
              bind:value={holding_institution}
              class="form-input"
              placeholder="e.g. Bibliothèque nationale de France"
            />
          </label>
          <label class="form-label">
            ...
            <input
              ...
              class="form-input"
              placeholder="e.g. Département Cartes et plans"
            />
          </label>
          <label class="form-label">
          ...
          <label class="form-label full-width">
            <span
              >Source URL <span class="field-hint">canonical record at the institution</span></span
            >
            <input
              type="url"
              bind:value={source_url}
              class="form-input mono"
              placeholder="https://gallica.bnf.fr/ark:…"
            />
          </label>
          <label class="form-label full-width">
            ...
            <input
              ...
              bind:value={rights}
              class="form-input"
              placeholder="e.g. Public domain, CC BY-SA 4.0"
            />
          </label>
        </div>
      {:else if activeTab === 'hosting'}
        <div class="hosting-section">
          ...
          <div
            class="hosting-subsection"
            style="background: var(--surface-2, #fafafa); padding: 0.75rem 1rem; border: 1px dashed var(--border, #ccc);"
          >
            <div class="subsection-heading" style="margin-bottom: 0.5rem;">
            ...
            <p style="margin: 0 0 0.5rem; font-size: 0.85rem; color: var(--text-muted, #666);">
              ...
              shelfmark, rights, language, source URL, holding institution). Existing values are
              never overwritten.
            </p>
            <button
              type="button"
          ...
          <div class="hosting-subsection">
            ...
            {#if orphanR2}
              <div class="orphan-warning">
                ⚠ maps.iiif_image points to R2 but no R2 source row exists — click "Mirror to R2"
                below to re-sync.
              </div>
            {/if}
                  ...
                  <div class="source-row" class:source-row--primary={src.is_primary}>
                    ...
                    <div class="source-actions">
                      {#if !src.is_primary}
                        <button
                          class="btn btn-outline btn-sm"
                          on:click={() => handleSetPrimary(src.id)}
                        >
                          Set primary
                        </button>
                      {/if}
                      <button
                        ...
                        on:click={() => handleDeleteSource(src.id)}
                      >
                        Remove
                      </button>
                    </div>
          ...
          <div class="hosting-subsection">
            ...
            <p class="section-desc">
              Clones the Allmaps annotation to Supabase Storage and sets <code
                >iiif.maparchive.vn</code
              > as the primary tile source. After clicking, run the printed CLI command to upload tiles.
            </p>
            {#if mirrorError}
          ...
          <div class="hosting-subsection">
            ...
            <label class="form-label full-width">
              <span
                >Annotation URL override <span class="dim"
                  >— optional, used when self-hosting via R2</span
                ></span
              >
              <input
                type="text"
