...
<script lang="ts">
  ...
  $: groups = (() => {
    if (groupBy === 'none')
      return [{ label: null, rows: sorted }] as { label: string | null; rows: any[] }[];
...
<style>
  ...
  .ct.compact th:nth-child(1),
  .ct.compact td:nth-child(1),
  .ct.compact th:nth-child(4),
  .ct.compact td:nth-child(4),
  .ct.compact th:nth-child(5),
  .ct.compact td:nth-child(5),
  .ct.compact th:nth-child(6),
  .ct.compact td:nth-child(6),
  .ct.compact th:nth-child(7),
  .ct.compact td:nth-child(7) {
    display: none;
  }
  ...
  .ct.compact tbody tr:last-child {
    border-bottom: none;
  }
  .ct.compact tbody td {
    padding: 0;
  ...
  .ct.compact td:nth-child(2) {
    order: 2;
    flex: 1 1 auto;
    min-width: 0;
  }
  .ct.compact td:nth-child(3) {
    order: 1;
    flex: 0 0 auto;
  }
  /* Title secondary in compact mode — name + creator stacked, smaller. */
  .ct.compact .title-col {
    flex-direction: column;
    align-items: flex-start;
    gap: 0.1rem;
    min-width: 0;
  }
  .ct.compact .title-row {
    min-width: 0;
    max-width: 100%;
  }
