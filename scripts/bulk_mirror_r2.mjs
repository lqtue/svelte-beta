...
function runTileMap(mapId, downloadUrl, originalIiif) {
  return new Promise((resolve, reject) => {
    ...
    p.on('close', (code) => {
      ...
      if (code === 0) resolve();
      else reject(new Error(`tile_map.sh exited ${code}`));
    });
  });
}
...
async function mirrorDb(map) {
  const annotationUrl =
    map.annotation_url ?? `https://annotations.allmaps.org/images/${map.allmaps_id}`;
  const r = await fetch(annotationUrl + '?_t=' + Date.now(), {
  ...
  const upRes = await fetch(
    `${SUPABASE_URL}/storage/v1/object/${ANNOTATIONS_BUCKET}/${storagePath}`,
    {
      method: 'POST',
      headers: {
