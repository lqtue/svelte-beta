...
function flattenIIIFv2(manifest) {
  const out = {
    _label: typeof manifest.label === 'string' ? manifest.label : null,
...
function parseBnF(manifest) {
  ...
  if (shelfmark)
    shelfmark = shelfmark
      .replace(/^Bibliothèque nationale de France,?\s*/, '')
      .replace(/^département\s+/, '');
  ...
  if (creator)
    creator = creator
      .split(' | ')[0]
      .replace(
        /\.\s*(Auteur du texte|Collaborateur|Dessinateur|Graveur|Cartographe|Éditeur|éditeur)\s*$/i,
        ''
      )
      .trim();
  ...
  if (format)
    format = format
      .split(' | ')
      .filter((x) => !x.match(/^image\/|Nombre total/))
      .join(' ; ');
  return {
    holding_institution: 'Bibliothèque nationale de France',
    collection:
      m['Repository'] === 'Bibliothèque nationale de France' &&
      m['Shelfmark']?.includes('Cartes et plans')
...
function parseIAOriginUrl(srcUrl) {
  ...
  if (srcUrl.includes('gallica.bnf.fr'))
    return { holder: 'Bibliothèque nationale de France', slug: 'bnf' };
  if (srcUrl.includes('maps.lib.utexas.edu'))
    return {
      holder: 'University of Texas at Austin, Perry-Castañeda Library Map Collection',
      slug: 'utaustin-pcl',
    };
  if (srcUrl.includes('humazur.univ-cotedazur.fr'))
    return { holder: "Humazur, Université Côte d'Azur", slug: 'humazur' };
  if (srcUrl.includes('blogs.loc.gov') || srcUrl.includes('loc.gov'))
    return { holder: 'Library of Congress', slug: 'loc' };
  if (srcUrl.includes('virtual-saigon.net') || srcUrl.includes('virtualsaigon'))
    return { holder: 'Virtual Saigon (IRD)', slug: 'virtual-saigon' };
  if (srcUrl.includes('wikipedia.org') || srcUrl.includes('wikimedia.org'))
    return { holder: 'Wikimedia Commons', slug: 'wikimedia' };
  if (srcUrl.includes('davidrumsey.com'))
    return { holder: 'David Rumsey Map Collection', slug: 'rumsey' };
  return { holder: null, slug: 'unknown', source_url_host: new URL(srcUrl).hostname };
}

// ---- Fetch all maps ----
const cols =
  'id,name,source_type,iiif_manifest,iiif_image,source_url,original_title,creator,dc_publisher,year_label,year,shelfmark,rights,language,physical_description,collection';
const filter =
  SOURCE_FILTER === 'all' ? 'source_type=in.(bnf,ia)' : `source_type=eq.${SOURCE_FILTER}`;
...
for (let i = 0; i < maps.length; i++) {
  ...
  process.stdout.write(
    `  [${i + 1}/${maps.length}] ${m.source_type} ${m.name.slice(0, 55).padEnd(55)} ... `
  );
  const entry = {
    id: m.id,
    name: m.name,
  ...
  if (m.source_type === 'bnf' && m.iiif_manifest) {
    ...
    entry.fetched = {
      ia_uploaded_metadata: iaMeta.metadata
        ? {
            ...
            description:
              typeof iaMeta.metadata.description === 'string'
                ? iaMeta.metadata.description.slice(0, 200)
                : null,
    ...
    if (origin?.slug === 'bnf') {
      ...
      if (arkMatch) {
        const bnfManifest = await fetchJson(
          `https://gallica.bnf.fr/iiif/${arkMatch[0]}/manifest.json`
        );
  ...
  if (entry.fetched) {
    const f =
      entry.fetched.bnf_origin_data ||
      entry.fetched.humazur_origin_data ||
      entry.fetched.utaustin_origin_data ||
      entry.fetched;
    for (const k of [
      'original_title',
  ...
  console.log(
    entry.errors.length
      ? `ERR: ${entry.errors.join(';')}`
      : `${Object.keys(entry.diff).length} diffs`
  );
...
for (const e of report)
  for (const k of Object.keys(e.diff)) fieldCounts[k] = (fieldCounts[k] || 0) + 1;
...
if (errors.length) {
  ...
  for (const e of errors)
    console.log(`  ${e.id.slice(0, 8)} ${e.name.slice(0, 40)} :: ${e.errors.join('; ')}`);
}
