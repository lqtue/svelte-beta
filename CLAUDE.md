...
**Adding a migration** — drop a new `supabase/migrations/NNN_*.sql` (incrementing from the current head, **071**), `supabase db push`, then regenerate types: `supabase gen types typescript --linked 2>/dev/null > src/lib/data/supabase/types.ts`. Run `npm run check` to catch fallout.
...
Schema: `supabase/migrations/` (head **071**). Table-by-table reference and the rules behind each constraint: **`docs/db-guidelines.md`** §11. The rules in one breath:
