...
The home page's hero is a **live map**, not an image: `src/lib/features/explore/HeroMap.svelte` mounts its own `MapShell` on an idle callback and plays four beats — the modern city, the 1882 Plan Cadastral warping over it, its traced footprints, its validated OCR labels. **The masthead is not a beat.** The page paints it, and the search field in it, immediately and lets the map arrive behind; it used to be the fifth beat, which meant the only call to action on the front page showed up about eight seconds after the reader did. `HeroSequence.svelte` beside it drives the cues on `setTimeout` and the fades on rAF (they shared one rAF loop until a throttled tab delayed the last cue). It creates **its own** map and layer stores rather than the persisted globals — a hero that wrote to `layersStore` would rearrange the reader's /explore layer stack. The sheet is hardcoded because it is the only one carrying all three layers.

**The masthead is a left column and the opening frame is pinned.** `HERO_SHEET` on the page carries `id` and a `view` of `{ lng, lat, zoom, rotation }` — all four chosen, where it used to be the sheet's bbox centre at a fixed zoom (which framed the whole sheet, margins included). Both come off an **/explore URL**, which is the only tool needed for either: set the camera there (⌘/ctrl-drag rotates), copy the address bar — `?map=<id>#@<lat>,<lng>,<zoom>z,<rotation>r` — and `id` is that `map=` while `view` is those four numbers, lat and lng swapped. The annotation URL is derived from `id` inside `HeroMap`, so changing which sheet plays is one uuid; the sheet has to be georeferenced **and** mirrored, since the hero plays our own copy.

The sheet sits right of the column through OL's own **view padding** (40% of the hero's width, capped at 600px), not an offset centre — the view is rotated, so an offset there is diagonal. `HeroMap` restores the centre after writing the padding, because OL's setter shifts the centre to keep on-screen content still: without that, every round trip through `HERO_SHEET.view` drifted the frame by half the padding. Legibility over the map is an ink wash down the left of `.hero-map-scrim` (pinned to `--light-ink`, like the rest of it) with the column taking `.on-ink-plate`'s paper ink over it; the search field re-pins to the light face with `.on-light-plate`, being a paper plate on that dark ground. The opacity slider lives **in the column**, under the Try chips, so it shares the left edge by layout rather than by a matching `clamp()` — which is why `overlayOpacity` and `settled` are bindable props of `HeroMap` rather than its own state. There are no zoom buttons; ⌘/ctrl-wheel zooms.

Five things keep the cost of a decorative map down. **The route imports it dynamically** (`(editorial)/+page.svelte`, on an idle callback, and only when `isMeteredConnection()` in `$lib/core/utils/connection.ts` says no) — a static import put OpenLayers, ol-pmtiles and Allmaps in the front page's first chunk, 179 kB of JS with ~390 kB of basemap behind it, before the masthead had painted; a metered reader now fetches 338 kB and 63 requests where the full hero is 1.1 MB and 93. That check lives in `core` rather than inside `HeroMap` because the point of a dynamic import is a decision made *before* the chunk is fetched. `MapShell` takes a `pixelRatio` prop and the hero pins it to **1** (at a Retina screen's own ratio OL asks for about four times the tiles — 71 requests becomes ~280). A `vma-hero-played-v1` session flag composes the final frame at once on a reload instead of replaying. The first beat waits for OL's `rendercomplete` (capped at 3 s) so a caption never claims something the frame has not painted yet. And **the fabric is frozen**: `heroFabric.ts` is generated by `scripts/gen-hero-fabric.mjs` and carries the sheet's footprints and labels as a module (5.9 kB gzipped) instead of a `/api/export/footprints` call and a Supabase query on every visit — `FootprintsLayer` takes it through a `featureCollection` prop and skips both the fetch and the un-warped filter the generator already applied, and the third and fourth captions quote `HERO_FOOTPRINT_COUNT` / `HERO_LABEL_COUNT` so the copy cannot drift from what is drawn. Re-run the script when the counts move; `tests/hero-fabric.spec.ts` is what fails if a regeneration comes back empty or in pixel coordinates.
