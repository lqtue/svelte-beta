...
`src/app.html` loads **Inter as a variable axis** (`wght@400..800`) rather than four static cuts — one file per unicode subset instead of four, 216 kB against 663 kB for the same pairing served statically. Be Vietnam Pro has no variable build, so it stays static at 600/700/800. The Google Translate widget that used to sit there is **gone**: ~100 kB of third-party script on every page for a control that never rendered (`autoDisplay: false`, hidden mount point). Do not reintroduce it without a visible language switch to justify it.

**Supabase types:**

- Insert/Update types: use `?:` optional fields — **not** `Partial<{...}>` (resolves as `never`).
- `src/lib/data/supabase/types.ts` is current against migration head 078 (`label_w`/`label_h` from 076; `global_xi`/`global_yi`, non-null `global_x`/`global_y` and `set_triage_key` from 077; `scout_candidates.review_note` from 078). 076, 077 and 078 are all pushed, and the file was regenerated from the linked project — the earlier note that 076/077 were unpushed and 077 hand-applied is stale. It had drifted: the file still declared `legend_submissions`, `map_help_requests`, `metadata_submissions` and `story_progress` long after 075 dropped them, because nothing regenerates it automatically — do it after every push. Prefer the real types over `as any`; ~25 casts remain, mostly in Svelte components.
...
Other persisted keys (there is no `vma-viewer-state-v1`): `vma-scout-view-v1`, `vma-layers-v1`, `vma-story-player-v1`, `vma-story-library-v1`, `vma-annotation-projects-v1`, `vma-bounds-cache-v2`, `vma-explore-sidebar-ratios-v2`, `vma-custom-base-url`, `vma-thumb-cache-v1`, `vma-explore-{tour,welcome}-ack-v1`, `vma-create-saigon-seeded-v2`. Debounced persistence lives in `src/lib/core/utils/persistence/createPersistedStore.ts`; raw read/write in the sibling `storage.ts`.
...
**The header demonstrates itself once.** `sweepHeroSlider()` on the page waits 1.1 s, then walks `heroSheet` from 1 down to 0.12 and back over 2.8 s on rAF — a triangle through `easeInOutCubic`, the curve the studio timeline already uses, so the turn at the bottom is not a corner. It stops dead on the reader's first `pointerdown` or `keydown` on the slider, runs once a tab (`vma-hero-swept-v1`), and never under `prefers-reduced-motion`. A slider nobody drags is a slider nobody knows is there; this one is the archive's whole idea, so it shows what it does and then leaves the control alone. It is an introduction, not an idle animation — do not make it loop.

Neither is a hand-made crop — `scripts/gen-hero-still.mjs` drives the real demo section in Playwright and photographs it twice, so the two ends of the slider line up to the pixel and both match the live map below (the 1882 one doubles as the section's own poster while OL loads). The two passes are separate page loads, and the `hero-now` one **aborts the annotation request on purpose**: `HeroSequence.start()` catches a failed overlay by parking at stage 0 and returning, so the sheet, footprints and labels never arrive and there is no 1.5-second window to race between "the basemap painted" and "the sheet starts fading in". **Re-run it whenever `HERO_SHEET` changes or the fabric is regenerated**, or the header advertises a frame the demo no longer opens on. It needs `vips` and `cwebp`, a dev server (or `HERO_BASE_URL`), and it hides the sticky `.top-nav` and OL's controls before shooting — the nav is sticky, so Playwright photographs it along with the map otherwise.
...
- **`ArchiveBrowser.svelte`** + **`ArchiveMapRows.svelte`** — the archive browser (shared catalog engine + facets) and its rows. They were `explore/Explore{ArchiveBrowser,MapRows}` until Sept 2026, when the /scan rail needed the same picker. `ArchiveMapRows` is a toggle against `layersStore` by default; pass `activeIds` and it becomes a radio over the caller's own selection, which is what a /scan tool (one open sheet, no layer stack) needs. `badges` puts caller-supplied text where the type chip goes — the pass progress on /scan?mode=triage, the pending count on `?mode=review` — because the catalog rows carry neither.
...
`src/lib/features/explore/ExploreSidebar.svelte` is the **left** rail, the archive: **Browse → Layers**, one draggable splitter, default 50/50, ratios persisted under `vma-explore-sidebar-ratios-v2` (the v1 value is three numbers, hence the bump). Its Browse pane is `ExploreBrowsePanel.svelte` + `ExploreArchiveBrowser.svelte`, not `CatalogSidebarPanel`.

`src/lib/features/explore/ExploreRightSidebar.svelte` is the **right** rail, the sheet on top of the stack — three tabs over `ToolLayout`'s existing `right-sidebar` slot, so it gets drag-resize and collapse for nothing: **Legend** (the numbered-legend index off `/api/maps/[id]/legend-points`, each row a place to fly to — the same GET `LegendPointsLayer` makes, so an open tab fetches it twice), **Controls** (`LayerControlsPanel`, unchanged) and **Info** (the top overlay's catalogue metadata). The tab strip is `.sb-pill`, not a fourth tab system. `data-tour="controls"` moved with the panel so the tour still lands on it.
...
**Review (`/scan?mode=review`)** — a queue per kind of contribution, chosen by a tab (`?kind=stories` opens the second one). **Stories**: `StoryReviewPanel.svelte` lists submitted stories and approves / sends back / rejects through `/api/admin/stories`. **Footprints**: HITL for SAM2 `submitted` / `needs_review` polygons — `ReviewPage.svelte` itself mounts `ToolLayout` + `ScanLeftRail` + `ImageShell` + `ReviewTool.svelte`, with `ReviewSidebar.svelte` (approve/reject, "Mark seg reviewed") in the right-sidebar slot. The queue is `fetchMapsWithSubmittedFootprints()` fed straight into the rail, one row per sheet badged with its pending count; the separate `ReviewMode.svelte` and its card-list picker page are gone. API `GET/PATCH /api/admin/footprints`; "Mark seg reviewed" PATCHes `/api/admin/maps/[id]/pipeline` → `seg_reviewed`.
...
Schema: `supabase/migrations/` (head **078**). Table-by-table reference and the rules behind each constraint: **`docs/db-guidelines.md`** §11. The rules in one breath:
