...
- **`TopSheetActions.svelte`** — the way out of the viewer for the sheet on top of the stack: `⬡ Traced` (the footprints toggle, whose state `ExplorePage` owns), Scan, Annotate, and Share for a published map only. It prints **no sheet name** — the panel header says whose it is and the Info tab carries the full one, so the label was an ellipsis eating the width four buttons needed. For the same reason the Info tab has **no "Catalogue page" link**: that is this strip's Share, four rows up the same panel. It sat at the foot of the layer stack in the **left** rail until Sept 2026; it now sits above the tab strip in `ExploreRightSidebar`, which is already the panel titled *This sheet*. Mobile has no right rail, so `ExplorePage` renders it under `LayerStackPanel` in the `mobile-layers` drawer.
...
`src/lib/features/explore/ExploreRightSidebar.svelte` is the **right** rail, and it is grouped **by subject**: the sheet on top of the stack, then the view onto the whole map. It rides `ToolLayout`'s existing `right-sidebar` slot, so it gets drag-resize and collapse for nothing. Head to foot: `TopSheetActions`; **two tabs, both about the sheet** — **Info** (its catalogue metadata, default) and **Legend** (the numbered-legend index off `/api/maps/[id]/legend-points`, each row a place to fly to — the same GET `LegendPointsLayer` makes, so an open tab fetches it twice); then a card titled **View** holding `LayerControlsPanel` (display mode, base map, place search, GPS). **View is not a tab**: a rail titled *This sheet* carried a Controls tab that was not about the sheet at all. Its wrapper `.view-sec` is `flex: 0 1 auto` with its own scroll, because `SidebarCard grow={0}` is `flex: 0 0 auto` and the rail is `overflow: hidden` — on a short viewport the GPS toggle would be cut off the bottom. The tab strip is `.sb-pill`, not a fourth tab system. `data-tour="controls"` stays on the `<aside>` so the tour still lands on it.
