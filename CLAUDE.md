...
**Adding a migration** — drop a new `supabase/migrations/NNN_*.sql` (incrementing from the current head, **075**), `supabase db push`, then regenerate types: `supabase gen types typescript --linked 2>/dev/null > src/lib/data/supabase/types.ts`. Run `npm run check` to catch fallout.
...
- `src/lib/data/supabase/types.ts` is current against migration head 075 (070 was the last to change any type; 071–074 only move rows and 075 drops four tables that never took one), verified identical to the linked project. Prefer the real types over `as any`; ~25 casts remain, mostly in Svelte components.
...
Schema: `supabase/migrations/` (head **075**). Table-by-table reference and the rules behind each constraint: **`docs/db-guidelines.md`** §11. The rules in one breath:
